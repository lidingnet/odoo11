...
class PurchaseTrack(models.Model):
    ...
    purchase_date= fields.Date(string="下单日期", compute ='_compute_purchase')
    ...
    purchase_back_date = fields.Date(string="合同回传", compute= '_compute_purchasee')
    ...
    def _compute_purchase(self):
        for rec in self:
            # 根据明细填写 下单日期
            rec.purchase_date = self.choose(rec, 1 )
            # 根据明细填写 实际交期
